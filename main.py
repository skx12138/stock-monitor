...
def main():
    ...
    while True:
        ...
        if is_trading_time():
            ...
            if (
                ...
                for c in close_candidates[:3]:
                    if c["code"] not in paper.portfolio.positions and c["score"] >= 55:
                        ...
                        if pred_adj >= 0:
                            buy_trade = paper._buy_position(c["code"], c["name"], price, 0.20,
            ...
            # ── AI买入条件推送（每小时一次） ──
            buy_alert_key = f"buy_{now.strftime('%Y%m%d_%H')}"
            if buy_analysis and buy_alert_key not in intraday_alerts:
                intraday_alerts.add(buy_alert_key)
                buy_lines = ["🤖 **AI买入条件扫描**", ""]
                buy_lines.extend(buy_analysis)
                buy_lines.append("")
                buy_lines.append("💡 评分≥45+板块+预测看涨可买入")
                notify(config, "🤖 AI买入分析", "\n".join(buy_lines))
                ...
                if drop_pct <= -2.0 and total_chg <= -1.5 and alert_key + "_drop" not in intraday_alerts:
                    ...
                    if should_sell:
                        ...
                        if sell_trade:
                            ...
                            batch_messages.append(f"  🔄 卖出 {display} {price:.2f}元×{sell_trade.shares}股{profit_str}")
                    else:
                        logger.info("跳水不卖出: %s", analysis_reason)
                        intraday_events.append(f"  ℹ️ {display} {analysis_reason}，暂不操作")

                # 深V检测：先跌超1.5%再反弹超1%
                if drop_pct <= -1.5 and rebound_pct >= 1.0 and abs(total_chg) < 0.5 and alert_key + "_vv" not in intraday_alerts:
                    ...
                    try:
                        ...
                        if not mc.get("tradable", True):
                            should_buy = False
                            analysis_reason = "大盘环境不适合"
                    except: pass
                    # 检查趋势 - 如果均线空头排列则不买
                    ...
                    if should_buy and code not in paper.portfolio.positions:
                        buy_trade = paper._buy_position(code, display, price, 0.20, analysis_reason)
                        if buy_trade:
                            batch_messages.append(f"  🔄 买入 {display} {price:.2f}元×{buy_trade.shares}股")

                # 急跌抄底：5分钟跌超3%+超卖+趋势完好
                if drop_pct <= -3.0 and alert_key + "_panic" not in intraday_alerts:
                    ...
                    if kline_panic is not None and len(kline_panic) > 20:
                        ...
                        # 超卖 + 趋势未破 + 仓位控制
                        if rsi_val is not None and rsi_val < 30 and above_ma60:
                            # 检查大盘不要跌太猛
                            try:
                    ...
                    if should_buy_panic and code not in paper.portfolio.positions:
                        buy_trade = paper._buy_position(code, display, price, 0.10, f"急跌{drop_pct:.0f}%·RSI{rsi_val:.0f}·抄底")
                        if buy_trade:
                            logger.info("急跌抄底: %s 跌%.1f%% RSI%.0f", display, abs(drop_pct), rsi_val)
                            batch_messages.append(f"  🔄 抄底买入 {display} {price:.2f}元×{buy_trade.shares}股")
            ...
            if important_msgs or intraday_events:
                merged_lines = []
                if important_msgs:
            ...
            # ── 盘后总结（15:00-15:05，每天一次） ──
            if not summary_done_today and now_time >= dt_time(15, 0) and now_time <= dt_time(15, 5):
        ...
        # 15:05后自动退出（盘后总结已发）
        if summary_done_today and now_time >= dt_time(15, 5):
            logger.info("15:05 已收盘，监控停止")
            break
